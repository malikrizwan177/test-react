...
const Products = () => {
  ...
  return (
    <div className="px-5 sm:px-10 md:px-14 lg:px-20 py-10 md:py-20 text-center">
        <h1 className="font-bold text-[40px]">Our Products</h1>
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 justify-center gap-y-10 gap-x-10 w-full mt-10">
            {products.map((item, index) => (
                <div key={index} className="max-w-[285px] bg-[#F4F5F7] text-start relative cursor-pointer" onClick={() => addToCart(item)}>
                    ...
                    <div className="absolute bg-slate-500/50 invisible hover:visible">
                        <button className="bg-white max-w-[200px] w-full py-2 px-5">Add to Cart</button>
                    </div>
                </div>
            ))}
        </div>
